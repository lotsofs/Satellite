...
public class GameProgression : MonoBehaviour {
    ...
    void Level1() {
        if (GameManager.score[(int)satTypes.Research] == 0 && GameManager.activeSatellites[(int)satTypes.Research] == 0) {
			...
			specialSat = satFactory.LaunchResearchSpecific(20, 3, factions.USA, 50, 25, 2, 10, false, false, false);
		}
        ...
        if (level1Progress == 2 && allSatsHaveSpawned == 0 && GameManager.subScore[(int)satTypes.Research] > GameManager.potentialScore[(int)satTypes.Research]) {
        ...
        if (level1Progress == 3 && allSatsHaveSpawned == 0 && GameManager.subScore[(int)satTypes.Research] > GameManager.potentialScore[(int)satTypes.Research]) {
            // All the satellites were destroyed and now you can't get more points. Launch a new one. 
            satFactory.LaunchResearchSpecific(40, 1, factions.USA, Random.Range(50, 100), 30, 2, 10, true, false, false);
        }
		if (level1Progress == 4 && (specialSat.derelict == true || (int)specialSat.launchStatus >= 5)) {
			// The one satellite integral to this level has gone derelict. Launch a new one.
			Debug.Log("DERELCTION)");
		}
